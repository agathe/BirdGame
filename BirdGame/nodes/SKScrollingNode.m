...
+ (id) scrollingNodeWithImageNamed:(NSString *)name inContainerWidth:(float) width
{
    ...
    SKScrollingNode * realNode = [SKScrollingNode spriteNodeWithColor:[UIColor clearColor] size:CGSizeMake(width, image.size.height * [UIScreen mainScreen].scale)];
    realNode.scrollingSpeed = 1;
    
    float total = 0;
    while(total<(width + image.size.width * [UIScreen mainScreen].scale)){
